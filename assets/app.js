...
const ICO = {
  ...
  wa:    `<svg class="ico-wa" viewBox="0 0 24 24" width="14" height="14"><path d="M12 2a10 10 0 0 0-8.6 15.1L2 22l5.1-1.3A10 10 0 1 0 12 2Z"/></svg>`
};
...
function aplicarAsesor() {
  const wa = `https://wa.me/${ASESOR.tel}`;
  const f = $("#fabWa"); if (f) f.href = wa;
  const b = $(".footer-btn.wa"); if (b) b.href = wa;
}
...
function bindEventos() {
  ...
  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape") { cerrarModal(); cerrarStock(); cerrarComparador(); if (tvTimer) tvStop(); }
...
function cardHTML(v, pos) {
  const foto = fotosList(v.fotos)[0], alt = `${esc(v.marca)} ${esc(v.modelo)}`;
  let media;
...
function galeriaHTML(fotos, alt) {
  ...
  const imgs = fotos.map((f, i) => `<img class="gal-img${i === 0 ? " active" : ""}" src="${esc(f)}" alt="${esc(alt)}" loading="${i === 0 ? "eager" : "lazy"}" decoding="async" referrerpolicy="no-referrer">`).join("");
...
function setGal(i) {
  ...
  imgs.forEach((im, n) => im.classList.toggle("active", n === galIndex));
  dots.forEach((d, n) => d.classList.toggle("active", n === galIndex));
}

/* ---------- 14. Radar (viewBox 220x200, tope 220px) ---------- */
function radarSVG(v, cls = "") {
  const { vals, labels } = perfil(v);
  const cx = 110, cy = 94, R = 58, n = 6;
  ...
  const vtx = vals.map((val, i) => { const [x, y] = pt(i, (R * val) / 10); return `<circle class="vtx" cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="3.5"><title>${labels[i]}: ${val}/10</title></circle>`; }).join("");
  const lbls = labels.map((l, i) => {
    const [x, y] = pt(i, R + 17);
    const an = Math.abs(x - cx) < 6 ? "middle" : x > cx ? "start" : "end";
    return `<text class="lbl" x="${x.toFixed(1)}" y="${y.toFixed(1)}" text-anchor="${an}" dy="3">${l}</text>
      <text class="val" x="${x.toFixed(1)}" y="${(y + 10).toFixed(1)}" text-anchor="${an}">${vals[i]}</text>`;
  }).join("");
  return `<svg class="radar ${cls}" viewBox="0 0 220 200" width="220" height="200" role="img" aria-label="Radar de cualidades">
      ${rings}${axes}<polygon class="shape" points="${shape}"/>${vtx}${lbls}</svg>`;
}
function radarHTML(v) {
  const { vals, labels, uso } = perfil(v);
  const legend = labels.map((l, i) => `<div class="rl-item"><span class="rl-name">${l}</span>
...
function fichaPDF(v) {
  const foto = fotosList(v.fotos)[0], p = precioTexto(v);
...
const agendaHTML = () => `<div class="agenda-panel" id="agendaPanel">
    <form class="agenda-form" id="agendaForm">
      ...
      <button class="btn-wa" type="submit">Enviar solicitud por WhatsApp</button>
    </form></div>`;
...
function toggleAgenda(v) {
  ...
  form.addEventListener("submit", (e) => {
    ...
    const msg = `¡Hola ${ASESOR.nombre}! Me llamo ${f.get("nombre")} (Tel: ${f.get("tel")}) y me gustaría agendar una visita para ver el ${nombreAuto(v)} el día ${ft} a las ${f.get("hora")} hs. ¿Me confirmás disponibilidad?`;
    track("agendar_visita", v);
    window.open(`https://wa.me/${ASESOR.tel}?text=${encodeURIComponent(msg)}`, "_blank");
...
function abrirModal(i) {
  ...
  $("#modalCard").innerHTML = `<button class="modal-close" data-close>&times;</button>
    ${galeriaHTML(fotosList(v.fotos), `${v.marca} ${v.modelo}`)}
    <div class="modal-content">
      ...
      <div class="modal-actions">
        <a class="btn-wa" data-act="wa" href="${wa}" target="_blank" rel="noopener">Consultar por WhatsApp</a>
        <button class="btn-agenda" data-act="agenda">📅 Agendar visita / Test Drive</button>
  ...
  const acc = {
    wa: () => track("consultar_wa", v), share: () => compartirFicha(v), qr: () => toggleQR(v),
...
function abrirComparador() {
  ...
  const head = (v) => {
    const f = fotosList(v.fotos)[0], p = precioTexto(v);
...
/* ---------- 22. UI ---------- */
function initUI() {
  detectarAsesor(); renderAsesorBadge(); aplicarAsesor();
